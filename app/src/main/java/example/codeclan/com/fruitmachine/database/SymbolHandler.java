...
public class SymbolHandler extends DatabaseHandler
{
    ...
    public ArrayList<Symbol> getAllSymbolsFromSet(String imageSetName) {
        ...
        String sql = String.format("SELECT * FROM %s WHERE %s = '%s'", SymbolDb.TABLE_SYMBOLS.toString(), SymbolDb.COL_ID.toString(), imageSetName);
        SQLiteDatabase db = this.getReadableDatabase();
        ...
        if (cursor.moveToFirst()){
            do {
                ...
                symbol.setImageSetName(cursor.getString(1));
                symbol.setImageName(cursor.getString(2));
                symbol.setImageLocation(cursor.getString(3));
                symbol.setScore(Integer.parseInt(cursor.getString(4)));
